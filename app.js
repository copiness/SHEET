(function () {
  ...
  function convertCellNameToRowCol(cellName) {
    ...
    for (let i = 0; i < columnLetters.length; i++) {
      colNum =
        colNum * 26 + (columnLetters.charCodeAt(i) - "A".charCodeAt(0) + 1);
    }
    return {
      row: rowNum,
      col: colNum,
  ...
  function run_avg_func(paramString, outputCellName) {
    ...
    if (isFinite(s)) {
      ...
      let numCells =
        (Math.abs(c1.row - c2.row) + 1) * (Math.abs(c1.col - c2.col) + 1);
      return s / numCells;
    }
    return "#NaN";
  }
  ...
  function run_sum_func(paramString, outputCellName) {
    ...
    for (let r = Math.min(c1.row, c2.row); r <= Math.max(c1.row, c2.row); r++) {
      for (
        let c = Math.min(c1.col, c2.col);
        c <= Math.max(c1.col, c2.col);
        c++
      ) {
        // add dependency
  ...
  function execute(expr, outputCellName) {
    ...
    const exprWithVals = expr.replace(cellNameRegex, (match) => {
  ...
  function recalculateDependents(selectedCellName) {
    if (selectedCellName in executionsDependentOnCell) {
      for (
        let i = 0;
        i < executionsDependentOnCell[selectedCellName].length;
        i++
      ) {
        const cellNameToBeExecuted =
          executionsDependentOnCell[selectedCellName][i];
  ...
  function resetTextInput() {
    if (textInput.style.display == "block") {
      ...
      const selectedCellName = convertRowColToCellName(
        selectedCell.row,
        selectedCell.col
      );
  ...
  function updateCellAtTheEndOfTextInput() {
    ...
    if (cellNames != null && cellNames.length > 0) {
      ...
      if (textInput.innerHTML.endsWith(lastCellName)) {
        ...
        let val = textInput.innerHTML.slice(
          0,
          textInput.innerHTML.length - lastCellName.length
        );
        val += convertRowColToCellName(
          selectedCellFormula.row,
          selectedCellFormula.col
        );
      ...
      textInput.innerHTML += convertRowColToCellName(
        selectedCellFormula.row,
        selectedCellFormula.col
      );
    }
  ...
  function displayInput() {
    const pos = DrawFunctions.getCellPosition(
      selectedCell.row,
      selectedCell.col
    );
  ...
  function handleDeleteKey() {
    ...
    if (
      ...
      for (
        let r = Math.min(selectedCellRange.startRow, selectedCellRange.endRow);
        r <= Math.max(selectedCellRange.startRow, selectedCellRange.endRow);
        r++
      ) {
        for (
          let c = Math.min(
          ...
          c <= Math.max(selectedCellRange.startCol, selectedCellRange.endCol);
          c++
        ) {
